...
SpectrumAnalyzer::SpectrumAnalyzer (SurgicalDeEsserAudioProcessor& p) : proc (p)
{
    startTimerHz (30);

    for (auto* btn : { &showSpectrumButton, &showEqCurveButton })
    {
        btn->setColour (juce::ToggleButton::textColourId, Palette::text);
        addAndMakeVisible (btn);
    }
    showSpectrumButton.setToggleState (true, juce::dontSendNotification);
    showSpectrumButton.setColour (juce::ToggleButton::tickColourId, Palette::cyan);

    showEqCurveButton.setToggleState (true, juce::dontSendNotification);
    showEqCurveButton.setColour (juce::ToggleButton::tickColourId, juce::Colour (0xffe8a33d)); // naranja, distinto al cian del espectro

    showSpectrumButton.onClick = [this] { repaint(); };
    showEqCurveButton.onClick  = [this] { repaint(); };
}

void SpectrumAnalyzer::resized()
{
    showSpectrumButton.setBounds (getWidth() - 190, 4, 90, 20);
    showEqCurveButton.setBounds (getWidth() - 95, 4, 90, 20);
}
...
void SpectrumAnalyzer::paint (juce::Graphics& g)
{
    ...
    if (showSpectrumButton.getToggleState())
    {
    ...
    // Curva de respuesta del EQ de voces, superpuesta en color distinto (naranja).
    // Es la curva TEÓRICA del filtro (no el audio en vivo) — igual que en FabFilter.
    if (showEqCurveButton.getToggleState())
    {
        ...
        // Línea de 0dB de referencia para la curva del EQ
        g.setColour (eqColour.withAlpha (0.18f));
        g.drawHorizontalLine ((int) eqDbToY (0.0f), bounds.getX(), bounds.getRight());
        ...
        {
            auto frac = (float) i / (float) numPoints;
            auto logMin = std::log10 (20.0f), logMax = std::log10 (20000.0f);
            auto freq = std::pow (10.0f, logMin + frac * (logMax - logMin));

            auto db = proc.getEqResponseDb (freq);
            auto x = bounds.getX() + freqToX (freq, bounds.getWidth());
            auto y = eqDbToY (juce::jlimit (-eqRangeDb, eqRangeDb, db));

            if (! eqStarted) { eqCurve.startNewSubPath (x, y); eqStarted = true; }
            else eqCurve.lineTo (x, y);
        }
        g.setColour (eqColour.withAlpha (0.20f));
        g.strokePath (eqCurve, juce::PathStrokeType (4.0f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
        g.setColour (eqColour);
        g.strokePath (eqCurve, juce::PathStrokeType (2.0f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
    }
...
VocalEqBandControl::VocalEqBandControl (SurgicalDeEsserAudioProcessor& proc, int bandIndex)
{
    ...
    onButton.setColour (juce::ToggleButton::tickColourId, Palette::cyan);
    onButton.setColour (juce::ToggleButton::textColourId, Palette::text);
    ...
    onAttach = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment> (
        proc.apvts, SurgicalDeEsserAudioProcessor::eqBandOnParamId (bandIndex), onButton);
}
...
void VocalEqBandControl::resized()
{
    ...
    onButton.setBounds (r.removeFromTop (18).withSizeKeepingCentre (50, 18));
    auto knobs = r;
