...
juce::AudioProcessorValueTreeState::ParameterLayout SurgicalDeEsserAudioProcessor::createParameterLayout()
{
    ...
    for (int i = 0; i < NUM_VOCAL_EQ_BANDS; ++i)
    {
        ...
        params.push_back (std::make_unique<juce::AudioParameterBool>(
            eqBandOnParamId (i), juce::String (d.name) + " On", true));
    }
...
juce::dsp::IIR::Coefficients<float>::Ptr SurgicalDeEsserAudioProcessor::computeBandCoefficients (int i) const
{
    const auto& d = kVocalEqBands[i];
    ...
    switch (d.type)
    {
        case VocalEqBandType::HighPass:
            return juce::dsp::IIR::Coefficients<float>::makeHighPass (currentSampleRate, freq, d.q);
        case VocalEqBandType::HighShelf:
            return juce::dsp::IIR::Coefficients<float>::makeHighShelf (currentSampleRate, freq, d.q, gainLinear);
        case VocalEqBandType::Bell:
        default:
            return juce::dsp::IIR::Coefficients<float>::makePeakFilter (currentSampleRate, freq, d.q, gainLinear);
    }
}
...
void SurgicalDeEsserAudioProcessor::updateVocalEqFilters()
{
    for (int i = 0; i < NUM_VOCAL_EQ_BANDS; ++i)
    {
        bool bandOn = apvts.getRawParameterValue (eqBandOnParamId (i))->load() > 0.5f;
        *vocalEqBands[(size_t) i].state = bandOn
            ? *computeBandCoefficients (i)
            : *juce::dsp::IIR::Coefficients<float>::makeAllPass (currentSampleRate, 1000.0);
    }
}

float SurgicalDeEsserAudioProcessor::getEqResponseDb (float frequency) const
{
    bool eqBypassed = apvts.getRawParameterValue ("eqBypass")->load() > 0.5f;
    if (eqBypassed || currentSampleRate <= 0.0)
        return 0.0f;
    ...
    for (int i = 0; i < NUM_VOCAL_EQ_BANDS; ++i)
    {
        ...
        auto coeffs = computeBandCoefficients (i);
        totalMagnitude *= coeffs->getMagnitudeForFrequency ((double) frequency, currentSampleRate);
    }
